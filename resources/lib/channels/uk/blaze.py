...
def getPlaylist(mode, stream, streamKey, streamUvid):
    ...
    licParams['key'] = streamKey
    resp = urlquick.get(stream, params=licParams, max_age=-1)
...
def list_categories(plugin, item_id, **kwargs):
    ...
    for video_datas in root.iterfind(
            ".//div[@class='slider-card card bg-transparent border-0 rounded position-relative mb-0 card-default']"):
...
def list_videos(plugin, item_id, video_id, video_image, **kwargs):
    ...
    resp = urlquick.get(video_id, max_age=-1)
