...
def list_programs(plugin, item_id, **kwargs):
    resp = urlquick.get(URL_REPLAY + URL_PATH_KIDS,
...
def list_episodes(plugin, serial_path, serial_name, serial_image_url):
    resp = urlquick.get(URL_REPLAY + serial_path,
                        headers={'User-Agent': web_utils.get_random_ua()},
                        max_age=-1, timeout=30, raise_for_status=False)
    if resp.status_code != 200:
        return False
...
def play_episode(plugin, episode_path):
    content = urlquick.get(URL_REPLAY + episode_path, headers={'User-Agent': web_utils.get_random_ua()},
                           max_age=-1, timeout=30, raise_for_status=False)
