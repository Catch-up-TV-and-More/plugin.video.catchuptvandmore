...
URL_API_MEDIA = 'http://api.allocine.fr/rest/v3/media?code=%s&partner=%s&format=json'
# videoId, PARTENER
...
CATEGORIES = {
    ...
    'Videos Films (Bandes-Annonces, Extraits, ...)' : URL_ROOT + '/video/films/',
    'Videos Séries TV  (Bandes-Annonces, Extraits, ...)' : URL_ROOT + '/series/video/'
}
...
def list_shows(params):
    ...
    if params.next == 'list_shows_emissions_1':
        ...
        replay_categories_programs_html = utils.get_webcontent(params.category_url)
        replay_categories_programs_soup = bs(replay_categories_programs_html, 'html.parser')
        root_categories_programs = replay_categories_programs_soup.find('li', class_='item_4 is_active ')
        replay_categories_programs = root_categories_programs.find_all('a')

        for category_programs in replay_categories_programs:
            categorie_programs_title = category_programs.get_text().strip().encode('utf-8')
        ...
        replay_subcategories_programs_html = utils.get_webcontent(params.categorie_programs_url)
        replay_subcategories_programs_soup = bs(replay_subcategories_programs_html, 'html.parser')
        ...
        programs_url = params.categorie_programs_url.replace('/cat-','/prgcat-')
        ...
        # Subcategories
        subcategories = replay_subcategories_programs_soup.find('div', class_='nav-button-filter').find_all('a')

        for subcategory in subcategories:
            subcategorie_programs_title = subcategory.find('span', class_='label').get_text().encode('utf-8')
        ...
        programs_title = '# Les programmes'
        next_value = 'list_shows_emissions_4'
        programs_url = params.subcategorie_programs_url.replace('/cat-','/prgcat-')
        ...
        replay_programs_html = utils.get_webcontent(params.programs_url + '?page=%s' % params.page)
        replay_programs_soup = bs(replay_programs_html, 'html.parser')
        replay_programs = replay_programs_soup.find_all('figure', class_='media-meta-fig')

        for program in replay_programs:

            program_title = program.find('h2', class_='title ').find('span').find('a').get_text().strip().encode('utf-8')
            program_img = program.find('img').get('src')
            program_url = URL_ROOT + program.find('h2', class_='title ').find('span').find('a').get('href').encode('utf-8')
        ...
        if replay_programs_soup.find('div', class_='pager pager margin_40t') is not None:
            ## More programs...
            shows.append({
        ...
        replay_seasons_html = utils.get_webcontent(params.program_url + 'saisons/')
        replay_seasons_soup = bs(replay_seasons_html, 'html.parser')
        replay_seasons = replay_seasons_soup.find_all('h2', class_='fs18 d_inline_block margin_10r')

        if len(replay_seasons) > 0:

            for season in replay_seasons:
                season_title = season.find('a').find('span').get_text().strip()
                show_season_url = URL_ROOT + season.find('a', class_='no_underline').get('href').encode('utf-8')

                ## Get Last Page
                last_page = '0'
                info_show_season = utils.get_webcontent(show_season_url)
                info_show_season_pages = re.compile('<a href="(.*?)"').findall(info_show_season)
            ...
            season_title = replay_seasons_soup.find('div', class_='margin_20t margin_40b').find('a').get_text().strip().encode('utf-8')
            show_season_url = URL_ROOT + replay_seasons_soup.find('div', class_='margin_20t margin_40b').find('a').get('href').encode('utf-8')
        ...
        # Build All Types
        replay_types_films_series_html = utils.get_webcontent(params.category_url)
        replay_types_films_series_soup = bs(replay_types_films_series_html, 'html.parser')
        replay_types_films_series = replay_types_films_series_soup.find_all('div', class_='left_col_menu_item')[0]
...
def list_videos(params):
    ...
    if params.next == 'list_videos_films_series_1':
        ...
        replay_episodes_html = utils.get_webcontent(params.show_url + '?page=%s' % params.page)
        replay_episodes_soup = bs(replay_episodes_html, 'html.parser')
        episodes = replay_episodes_soup.find_all('article', class_="media-meta sidecontent small")

        for episode in episodes:
            video_title = episode.find('h2').find('span').find('a').get_text().strip().encode('utf-8')
            video_id = re.compile('cmedia=(.*?)&').findall(episode.find('a').get('href'))[0]
        ...
        if replay_episodes_soup.find('section', class_='media-meta-list by2 j_w') is not None:
            root_episodes_soup = replay_episodes_soup.find('section', class_='media-meta-list by2 j_w')
            episodes = root_episodes_soup.find_all('figure', class_='media-meta-fig')
        else:
            episodes = replay_episodes_soup.find_all('figure', class_='media-meta-fig')

        for episode in episodes:
            if episode.find('h3') is not None:
                video_title =  episode.find('h3').find('span').find('a').get_text().strip()
            else:
                video_title =  episode.find('h2').find('span').find('a').get_text().strip()
            if '?cmedia=' in episode.find('a').get('href'):
                video_id = episode.find('a').get('href').split('?cmedia=')[1]
            elif 'cfilm=' in episode.find('a').get('href') or 'cserie=' in episode.find('a').get('href'):
                video_id = episode.find('h2').find('span').find('a').get('href').split('_cmedia=')[1].split('&')[0]
            else:
                video_id = episode.find('a').get('href').split('-')[1].replace('/','')
            video_plot = ''
            for plot_value in episode.find('div', class_='media-meta-figcaption-inner').find_all('p'):
...
def get_video_url(params):
    ...
    video_json = utils.get_webcontent(URL_API_MEDIA % (params.video_id, PARTNER))
    ...
    if 'rendition' in video_json_parser["media"]:
        ...
        url_video_json = re.compile('data-model="(.*?)"').findall(url_video_embeded_html)[0].replace('&quot;','"')
        url_video_json_parser = json.loads(url_video_json)
        # Case Facebook, Youtube and Vimeo (Not Working HTTP 503)
        if 'facebook' in url_video_json or 'youtube' in url_video_json or 'vimeo' in url_video_json:
            url_ytdl = url_video_json_parser["videos"][0]["sources"]["code"]
            url_ytdl = re.compile('src=(.*?) ').findall(url_ytdl)[0].replace('\\','').replace('&amp;','&').replace('"','')
        # Case DailyMotion
        else:
            url_ytdl = url_video_json_parser["videos"][0]["sources"]["url_provider"]
            url_ytdl = 'https:' + url_ytdl.split(':')[1].replace('\\','').replace('&amp;','&').replace('"','')
        ydl = YoutubeDL()
