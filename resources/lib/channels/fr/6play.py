...
PUBLIC_SITE = 'https://www.6play.fr'
CUSTOMER_NAME = "m6web"
SERVICE_NAME = "6play"
...
URL_LICENCE_KEY = ('https://lic.drmtoday.com/license-proxy-widevine/cenc/'
                   ...
                   '&Host=lic.drmtoday.com&x-dt-auth-token=%s|R{SSM}|JBlicense')
# Referer, Token
...
LIVE_CHANNEL = {
    ...
    "teva": "teva",
    "NFL": "NFL",
    "konbini24_24": "konbini24_24",
    "telefilms24_24": "telefilms24_24",
    "telefilmsromance24_24": "telefilmsromance24_24",
    "zoneinterdite24_24": "zoneinterdite24_24",
    "enquetescriminelles24_24": "enquetescriminelles24_24",
    "telenovelas24_24": "telenovelas24_24",
    "leliveby6play": "leliveby6play"
}
...
def list_categories(plugin, item_id, **kwargs):
    ...
    resp = urlquick.get(URL_ROOT % get_root_param(item_id), headers=GENERIC_HEADERS)
    json_parser = resp.json()
...
def accounts_login(plugin, api_key):
    login = plugin.setting.get_string('6play.login')
    password = plugin.setting.get_string('6play.password')
    if login == '' or password == '':
        xbmcgui.Dialog().ok(
            plugin.localize(30600),
            plugin.localize(30604) % ('6play', ('%s' % PUBLIC_SITE)))
        return False, None, None, None

    headers = {
        'User-Agent': web_utils.get_random_ua(),
        "Content-Type": "application/x-www-form-urlencoded",
        'referer': ('%s/connexion' % PUBLIC_SITE)
    }
